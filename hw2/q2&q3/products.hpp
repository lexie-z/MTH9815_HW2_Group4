...
public:
    ...
    //Get Underlying Bond
    Bond* GetUnderlyingBond() const;
private:
    Bond* underlyingBond; // underlying bond
};
...
ostream& operator << (ostream &output, const Future &future) {
    output << "Product ID: " << future.GetProductId() << ", Ticker: " << future.ticker << ' ' << future.maturityDate;
    return output;
}

EuroDollarFuture::EuroDollarFuture(string _productId, date _maturityDate) : Future(_productId, "Eurodollar", _maturityDate) {}

BondFuture::BondFuture(string _productId, Bond* _underlyingBond, date _maturityDate) : Future(_productId, _underlyingBond->GetTicker(), _maturityDate), underlyingBond(_underlyingBond) {}

Bond* BondFuture::GetUnderlyingBond() const {
